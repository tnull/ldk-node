...
pub enum Event {
	...
	/// A payment for a BOLT 11 invoice with a user-provided payment hash has been received.
	///
	...
	///
	/// This event is only emitted for unknown BOLT 11 payments if
	/// [`Config::manually_handle_unknown_bolt11_payments`] is enabled. Enabling this may let any
	...
	pub async fn handle_event(&self, event: LdkEvent) -> Result<(), ReplayEvent> {
		match event {
			...
			LdkEvent::PaymentClaimable {
				...
				if payment_info.is_none() {
					...
					} = &purpose
					{
						if !self.config.manually_handle_unknown_bolt11_payments {
							log_info!(
								self.logger,
								"Refusing unknown BOLT11 payment with hash {} of {}msat",
								hex_utils::to_string(&payment_hash.0),
								amount_msat,
							);
							self.channel_manager.fail_htlc_backwards(&payment_hash);
				...
				let should_insert_payment = payment_info.is_none();
