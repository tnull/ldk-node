...
impl<'a, K: Clone + Eq + Hash, T> PendingRequestGuard<'a, K, T> {
	pub(crate) fn insert(
		...
		pending_requests_lock
			.insert(request_key.clone(), PendingRequest { token: Arc::clone(&token), sender });
		...
		.find(|lsp_node| {
			if let Some(override_node_id) = override_node_id {
				lsp_node.node_id == *override_node_id
					&& lsp_node.supported_protocols.as_ref().is_some_and(|p| p.contains(&protocol))
			} else {
				lsp_node.supported_protocols.as_ref().is_some_and(|p| p.contains(&protocol))
			}
		})
...
impl Liquidity {
	pub(crate) fn new(
		...
		connection_manager: Arc<ConnectionManager<Arc<Logger>>>,
		liquidity_source: Arc<LiquiditySource<Arc<Logger>>>, logger: Arc<Logger>,
	) -> Self {
...
impl Liquidity {
	...
	pub fn lsps1(&self) -> LSPS1Liquidity {
		LSPS1Liquidity::new(
			...
			self.liquidity_source.lsps1_client(),
	...
	L::Target: LdkLogger,
{
	...
	pub(crate) async fn build(self) -> Result<LiquiditySource<L>, BuildError> {
		...
		Ok(LiquiditySource {
			...
			lsps1_client: Arc::new(LSPS1Client {
				lsp_nodes: Arc::clone(&lsp_nodes),
				pending_opening_params_requests: Mutex::new(HashMap::new()),
				pending_create_order_requests: Mutex::new(HashMap::new()),
				pending_check_order_status_requests: Mutex::new(HashMap::new()),
	...
	L::Target: LdkLogger,
{
	...
	pub(crate) fn lsps1_client(&self) -> Arc<LSPS1Client<L>> {
		...
		match self.liquidity_manager.next_event_async().await {
			...
			LiquidityEvent::LSPS0Client(LSPS0ClientEvent::ListProtocolsResponse {
				...
				if self.is_lsps_node(&counterparty_node_id) {
					...
					{
						match request.sender.send(protocols) {
							Ok(()) => (),
							Err(_) => {
								log_error!(
									self.logger,
	...
	pub(crate) async fn discover_lsp_protocols(
		...
		let _pending_request = {
			...
			if pending_discovery.contains_key(node_id) {
				log_error!(
					self.logger,
		...
		if let Some(lsp_node) =
			self.lsp_nodes.write().expect("lock").iter_mut().find(|n| &n.node_id == node_id)
		{
			lsp_node.supported_protocols = Some(protocols.clone());
		}

		Ok(protocols)
	}

	pub(crate) async fn get_lsp_config(
		&self, node_id: &PublicKey, protocol: u16,
	) -> Option<LspConfig> {
		if let Some(node) = select_lsps_for_protocol(&self.lsp_nodes, protocol, Some(node_id)) {
			return Some(node);
		}
		...
		if has_undiscovered_protocol && !*self.discovery_done_rx.borrow() {
			log_debug!(
				self.logger,
				"LSP {} protocols not yet discovered, waiting for protocol discovery to complete.",
				node_id
			);
