...
const LSPS1_DEFAULT_REQUIRED_CHANNEL_CONF: u16 = 0;
	...
	L::Target: Logger,
{
	...
	pub(crate) async fn lsps1_request_channel(
		...
		let order_params = OrderParameters {
			lsp_balance_sat,
			client_balance_sat,
			required_channel_confirmations: LSPS1_DEFAULT_REQUIRED_CHANNEL_CONF,
		...
		if response.order_params != order_params {
			log_error!(
				self.logger,
				"Aborting LSPS1 request as LSP-provided parameters don't match our order."
			);
			return Err(Error::LiquidityRequestFailed);
		}
