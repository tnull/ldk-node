...
fn channel_full_cycle() {
	...
	let node_a = NodeBuilder::from_config(config_a).set_esplora_server(esplora_url.clone()).build();
	node_a.start().unwrap();

	println!("\n== Node B ==");
	let config_b = random_config();
	let node_b = NodeBuilder::from_config(config_b).set_esplora_server(esplora_url).build();
	node_b.start().unwrap();
...
fn channel_full_cycle_0conf() {
	...
	let node_b = NodeBuilder::from_config(config_b).set_esplora_server(esplora_url.clone()).build();
...
fn connect_to_public_testnet_esplora() {
	...
	let node = NodeBuilder::from_config(config)
...
fn start_stop_reinit() {
	...
	let node =
		NodeBuilder::from_config(config.clone()).set_esplora_server(esplora_url.clone()).build();
	...
	let reinitialized_node =
		NodeBuilder::from_config(config).set_esplora_server(esplora_url).build();
...
fn start_stop_reinit_fs_store() {
	...
	let esplora_url = format!("http://{}", electrsd.esplora_url.as_ref().unwrap());
	let config = random_config();
	let node = NodeBuilder::from_config(config.clone())
		.set_esplora_server(esplora_url.clone())
		.build_with_fs_store();
	let expected_node_id = node.node_id();
	...
	let reinitialized_node =
		NodeBuilder::from_config(config).set_esplora_server(esplora_url).build_with_fs_store();
	assert_eq!(reinitialized_node.node_id(), expected_node_id);
